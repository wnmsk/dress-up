use crate::{cbor::cbor_bstr_header, consts::{self, cbor::map, labels}};
...
pub fn build_manifest(data: &[u8]) -> Vec<u8> {
    if data.len() <= 70 {
        return data.to_vec(); // TODO: check for length in target itself
    }
    ...
    let vendor_id = &data[4..20];
    let class_id = &data[20..36];
    let img_hash = &data[36..68]; // TODO: maybe rather calculate hash here
    let img_size = [data[69], data[70]];
